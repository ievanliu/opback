...
class VMINFOAPI(Resource):
	...
	def __init__(self):
		self.reqparse = reqparse.RequestParser()
		self.reqparse.add_argument('pm_id', type = str, help='PM_ID cannot be converted')
		self.reqparse.add_argument('vm_name', type = str, help='VM_Name cannot be converted')
		self.reqparse.add_argument('ip', type = str, help='IP cannot be converted')
		self.reqparse.add_argument('creater_time', type = str, help='Creater_Time cannot be converted')
		self.reqparse.add_argument('vn_id', type = str, help='VN_ID cannot be converted')
		self.reqparse.add_argument('vm_status', type = int, help='VM_STATUS cannot be converted')
		...
		try:
			...
			if not old_vm:
				...
				new_vm = Vm_info_tab(vm_id=vm_id, pm_id=args['pm_id'], vm_name=args['vm_name'], 
									ip=args['ip'], creater_time=args['creater_time'], 
				...
				abort(403, message = ('VM %s Already Existed' % vm_id).format(str))
...
api.add_resource(VMINFOAPI, '/api/v0.0/vminfos/<string:vm_id>', endpoint = 'vminfo')
